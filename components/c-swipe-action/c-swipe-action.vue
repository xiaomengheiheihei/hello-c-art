<template>
	<view class="c-swipe-action">
		<view :class="{'c-swipe-action--show':isShowBtn}" :style="{'transform':transformX,'-webkit-transform':transformX}" class="c-swipe-action__container" @touchstart="touchStart" @touchmove="touchMove" @touchend="touchEnd" @touchcancel="touchEnd" @click="bindClickCont">
			<view class="c-swipe-action__content">
				<slot />
			</view>
			<view :id="elId" class="c-swipe-action__btn-group">
				<div v-for="(item,index) in options" :key="index" :style="{backgroundColor: item.style && item.style.backgroundColor ? item.style.backgroundColor : '#C7C6CD',color: item.style && item.style.color ? item.style.color : '#FFFFFF',fontSize: item.style && item.style.fontSize ? item.style.fontSize : '28upx'}" class="c-swipe-action--btn" @click="bindClickBtn(item,index)">
					{{ item.text }}
				</div>
			</view>
		</view>
		<view v-if="isShowBtn" class="c-swipe-action__mask" @click="close" @touchmove.stop.prevent="close" />
	</view>
</template>
...
<style>
	...
	.c-swipe-action__content {
		width: 50%
	}

	.c-swipe-action__btn-group {
		display: flex;
		flex-direction: row
	}
