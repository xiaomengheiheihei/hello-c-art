	...
	<view>
        ...
        rightTop: rightTop
          ...
          right: horizontal === 'right' && direction === 'horizontal'
				...
				<text :class="{ active: isShow }" class="c-icon c-icon-plusempty" />
			</view>
			<view :class="{
...
<script>
	export default {
		...
		computed: {
			contentWidth(e) {
				return c.upx2px((this.content.length + 1) * 110 + 20) + 'px'
			},
			contentWidthMin() {
				return c.upx2px(110) + 'px'
			},
			// 动态计算宽度
			boxWidth() {
...
<style scoped>
	@font-face {
		...
		font-weight: normal;
		font-style: normal;
		src: url('https://img-cdn-qiniu.dcloud.net.cn/fonts/c.ttf') format('truetype');
	}
	...
	.fab-circle .c-icon-plusempty.active {
