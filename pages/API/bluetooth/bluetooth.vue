<template>
	<view>
		...
		<view class="c-padding-wrap c-common-mt">
			...
			<view class="c-btn-v">
				...
				<view v-if="characteristicsData.length > 0">
					<view class="c-list_name">uuid:{{ characteristicsData[0].uuid }}</view>
					<view class="c-list_item">
		...
		<!-- 遮罩 -->
		<view v-if="maskShow" class="c-mask" @touchmove.stop.prevent="moveHandle" @click="maskclose">
			<scroll-view class="c-scroll_box" scroll-y @touchmove.stop.prevent="moveHandle" @click.stop="moveHandle">
				...
				>
					<view v-if="showMaskType === 'device'">
						<view class="c-list_name">{{ item.name || item.localName }}</view>
						<view class="c-list_item">信号强度:{{ item.RSSI }}dBm</view>
						<view class="c-list_item">UUID:{{ item.deviceId }}</view>
						<!-- <view class="list-item" v-if="showMaskType === 'device'">
							Service数量:{{ item.advertisServiceUUIDs.length }}
						</view> -->
					</view>
					<view v-if="showMaskType === 'service'">
						<view class="c-list_item" style="line-height:2.2;">
							UUID: {{ item.uuid }}
					...
					<view v-if="showMaskType === 'characteristics'">
						...
						<view class="c-list_item">是否支持 read 操作:{{ item.properties.read }}</view>
						<view class="c-list_item">
...
export default {
	...
	methods: {
		...
		onBluetoothDeviceFound() {
			c.onBluetoothDeviceFound(devices => {
		...
		onBLEConnectionStateChange() {
			c.onBLEConnectionStateChange(res => {
		...
		onBLECharacteristicValueChange() {
			// 必须在这里的回调才能获取
			c.onBLECharacteristicValueChange(characteristic => {
		...
		/**
		 * 订阅操作成功后需要设备主动更新特征值的 value，才会触发 c.onBLECharacteristicValueChange 回调。
		 */
