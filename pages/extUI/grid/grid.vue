<template>
	<view class="page">
		<view class="example">
			...
			<c-grid :options="data1" @click="onClick" />
			...
			<c-grid :options="data3" :show-out-border="false" />
			<view class="example-title">无所有框</view>
			<c-grid :options="data3" :show-border="false" />
			<view class="example-title">一行四个</view>
			<c-grid :options="data2" :show-out-border="false" :column-num="4" />
			<view class="example-title">矩形案例</view>
			<c-grid :options="data3" type="oblong" />
		</view>
	</view>
</template>

<script>
	import uniGrid from '@/components/c-grid/c-grid.vue'
	...
	export default {
		...
		onReady() {
			c.createSelectorQuery().select('.grid-view').boundingClientRect().exec((ret) => {
