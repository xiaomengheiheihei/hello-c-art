<template>
	<view>
		<view class="example">
			...
			<c-popup :show="type === 'top'" position="top" mode="fixed" msg="顶部弹出popup" @hidePopup="togglePopup('')" />
			<button type="button" @click="togglePopup('middle')">居中弹出 popup</button>
			<c-popup :show="type === 'middle'" position="middle" mode="fixed" msg="居中弹出popup" @hidePopup="togglePopup('')" />
			<button type="button" @click="togglePopup('bottom')">底部部弹出 popup</button>
			<c-popup :show="type === 'bottom'" position="bottom" mode="fixed" msg="底部弹出popup" @hidePopup="togglePopup('')" />
		</view>
		<view class="example">
			<view class="example-title">slot用法</view>
			<button type="button" @click="togglePopup('middle-img')">居中弹出（插屏广告）</button>
			<c-popup :show="type === 'middle-img'" position="middle" mode="insert" @hidePopup="togglePopup('')">
				<view class="c-center center-box">
					<image class="image" src="/static/c.png" />
				</view>
			</c-popup>
			<button type="button" @click="togglePopup('middle-list')">居中弹出（滚动列表）</button>
			<c-popup :show="type === 'middle-list'" position="middle" mode="fixed" @hidePopup="togglePopup('')">
				<scroll-view :scroll-y="true" class="c-center center-box">
					<view v-for="(item, index) in list" :key="index" class="c-list-item">
						滚动列表数据 {{ item }}
					</view>
				</scroll-view>
			</c-popup>
			<button type="button" data-position="bottom" @click="togglePopup('bottom-share')">底部弹出（分享界面）</button>
			<c-popup :show="type === 'bottom-share'" position="bottom" @hidePopup="togglePopup('')">
...
<script>
	import uniPopup from '@/components/c-popup/c-popup.vue'
