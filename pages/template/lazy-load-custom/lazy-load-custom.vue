<template>
	<view>
		<view class="c-padding-wrap">
			<view class="c-helllo-text" style="padding:30upx 0;">
		...
		<view class="c-list">
			<view class="c-list-cell" hover-class="c-list-cell-hover" v-for="(item,index) in list" :key="index">
				<view class="c-media-list">
					<view class="c-media-list-logo">
					...
					<view class="c-media-list-body">
						<view class="c-media-list-text-top">主标题</view>
						<view class="c-media-list-text-bottom c-ellipsis">列表二级标题</view>
					</view>
				</view>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			load() {
				c.createSelectorQuery().selectAll('.lazy').boundingClientRect((images) => {
		...
		onLoad() {
			this.windowHeight = c.getSystemInfoSync().windowHeight
		},
		onShow() {
			if (!this.show) {
				this.show = true
