<template>
	<view>
		...
		<button class="button" @click="openTaobao('taobao://s.taobao.com/search?q=c-app')">打开淘宝搜索页面</button>
		<button class="button" @click="openMap()">打开地图并指定地点</button>
		<view class="c-divider">
			<view class="c-divider__content">打开QQ</view>
			<view class="c-divider__line"></view>
		</view>
		<view class="c-padding-wrap">
			<form @submit="openQQ">
				<view>
					<view class="c-title">请输入聊天对象QQ号：</view>
					<view class="c-list">
						<view class="c-list-cell">
							<input class="c-input" name="qqNum" type="number"/>
						</view>
					</view>
				</view>
				<view>
					<view class="c-title">请选择QQ号类型：</view>
					<radio-group class="c-flex" name="qqNumType">
						<label>
	...
	methods: {
		...
		openMarket(marketPackageName) {
			...
			if (plus.os.name=="Android") {
				appurl = "market://details?id=io.dcloud.HelloH5"; //由于hello c-app没有上Android应用商店，所以此处打开了另一个示例应用
			}
			else{
				appurl = "itms-apps://itunes.apple.com/cn/app/hello-c-app/id1417078253?mt=8";
			}
			...
			plus.runtime.openURL(url, function(res) {
				c.showModal({
					...
					success:function(res){
						if (res.confirm) {
							plus.runtime.openURL("https://s.taobao.com/search?q=c-app")
						}
					}
				})
			});
		...
		openMap(){
			...
			if (plus.os.name=="Android") {
				...
				var urlBaiduMap = "baidumap://map/marker?location=39.968789,116.347247&title=DCloud&src=Hello%20c-app";
				var urlAmap = "androidamap://viewMap?sourceApplication=Hello%20c-app&poiname=DCloud&lat=39.9631018208&lon=116.3406135236&dev=0"
				...
				plus.nativeUI.actionSheet({title:"选择地图应用",cancel:"取消",buttons:[{title:"Apple地图"},{title:"百度地图"},{title:"高德地图"}]}, function(e){
					...
					switch (e.index){
						case 1:
							...
							break;
						case 2:
							url = "baidumap://map/marker?location=39.968789,116.347247&title=DCloud&src=Hello%20c-app";
							break;
						case 3:
							url = "iosamap://viewMap?sourceApplication=Hello%20c-app&poiname=DCloud&lat=39.9631018208&lon=116.3406135236&dev=0";
							break;
						default:
							break;
					}
