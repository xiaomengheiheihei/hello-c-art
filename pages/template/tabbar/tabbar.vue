<template>
	<view class="c-tab-bar">
		<scroll-view id="tab-bar" class="c-swiper-tab" scroll-x :scroll-left="scrollLeft">
...
<script>
	...
	const tpl = {
		...
		data3: {
			...
			"datetime": "5分钟前",
			"title": "c-app 支持使用 npm 安装第三方包，生态更趋丰富",
			"source": "DCloud",
			"comment_count": 11
		},
		data4: {
			"datetime": "2小时前",
			"article_type": 4,
			"title": "c-app 支持原生小程序自定义组件，更开放、更自由",
	...
	export default {
		...
		methods: {
			...
			async changeTab(e) {
				...
				let winWidth = c.getSystemInfoSync().windowWidth,
			...
			getElSize(id) { //得到元素的size
				return new Promise((res, rej) => {
					c.createSelectorQuery().select("#" + id).fields({
						size: true,
						scrollOffset: true
					}, (data) => {
						res(data);
